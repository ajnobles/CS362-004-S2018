...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      effectOfAdventurer(state, currentPlayer);
			
    case council_room:
      effectOfCouncilRoom(state, currentPlayer, handPos);
    ...
    case smithy:
      effectOfSmithy(state, currentPlayer, handPos);
    ...
    case sea_hag:
      effectOfSeaHag(state, currentPlayer);
...
int effectOfAdventurer(struct gameState *state, int currentPlayer)
{
  ...
  while(drawntreasure<2){
       ...
       *
       * !!BUG!!: changed '<1' to '>1'
       * This causes the deck to always get shuffled.
       * 
       * Original code:
       *
...
int effectOfSmithy(struct gameState *state, int currentPlayer, int handPos)
{
  ...
  /*******************************************************************
  *
  * !!BUG!!: changed 'i < 3' to 'i < 2'
  * This causes only 2 cards to be drawn when the card is played.
  *
  * Original code:
  *
  * for (i = 0; i < 3; i++)
...
int effectOfCouncilRoom(struct gameState *state, int currentPlayer, int handPos)
{
  ...
  /*******************************************************************
  *
  * !!BUG!!: changed 'state->numBuys++' to 'state->numBuys--'
  * This causes the player to lose instead of gain a buy action.
  *
  * Original code:
  *
  * //+1 Buy
...
int effectOfSteward(struct gameState *state, int currentPlayer, int handPos, int choice1, int choice2, int choice3)
{
  ...
  else
  {
    /*****************************************************************
    *
    * !!BUG!!: discard instead of trash the two chosen cards
    * The last parameter of discardCard determines the location where
    * the discarded card is sent. 1 = trash, 0 = discard pile
    *
    * Original code:
    *
